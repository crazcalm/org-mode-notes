...
I have gotten over my issue with being verbose. The new plan is that I will create a setter method for each property. This will work for me because almost every property is an ~Option~, which means the ~Default~ instance (I derived a Default implementation) of the structs is having all the properties default to `None`.

So, the idea is:
- I will be able to get and object from the database to populate the UI fields.
- The UI fields will control their own manipulation of stuff.
- Saving the data in the UI will work as follows:
    - We will create a default instance of the struct with all Nones for fields.
    - We will update this default instance with the data pulled from the UI
    - If this is a new database entry, we call the /create_entry/ method on the struct.
    - If it is an existing database entry, we get a new struct with the all the current database information. We call an update method where we pass in the struct with the UI information. This method will use the UI struct info to update the struct with the database information. Then we will call /update_entry/ on that struct to update the database entry.

 All in all, this plan requires that I:
 - Create setter methods for all properties
 - Create an /create_entry/ method
 - Create a /update_entry/ method

 I will use the Company sruct as the prototype. Once I prove it out the approach, I will port it to the rest of the structs.

** Updates (MVP -V2)
Now that I have a basic skeleton on the application, I want to update the database schema to be more useful and less bloated.
...
- Notes
  ...
  I don't think Company has to change right now, but I want to update Contacts to have boolean flags for:
  - recruiter
  - interviewer
  - other

  Notes should be updated to have foriegn key reference to Company, Contacts, of Job Posting.

  I hope this is all the information I need.

*** Questions
**** Migrations
I am early enough in the project that droping the tables and starting from scratch is doable, but I am curious as to what I have to properly alter the tables to do what I want them to.

***** Steps:
I need a migration to get rid of the tables and fields that I no longer want/need.

Looks like I am only dropping the recruiters table. This means that I need to drop the recruiter_id field from all the other tables.

Nope:
I can't use the alter table command to reach the table state I want because I cannot drop table columns that have a foreign key reference. I think that when I create the tables again, I will not use foreign key references at all. For sqlite, I think this will make alteration easier in the future.

**** Migrations V2
I dropped all foreign key references, dropped recruiters table, updated notes, job_posting and interview tables. I made a mistake and missed the recruiters field in on of the tables and was able to write a migration to alter the table and drop the column because it did not have a foreign key reference associated with it. That felt good.
